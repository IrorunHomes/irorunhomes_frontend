...
const menuItems: MenuItem[] = [
  ...
  { 
    ...
    children: [
      { id: 'properties-list', label: 'Properties List', icon: HomeModernIcon, path: '/dashboard/super-admin/properties' },
      { id: 'add-property', label: 'Add Property', icon: PlusCircleIcon, path: '/dashboard/super-admin/properties/add' },
    ]
  },
  { 
    id: 'rental-requests', 
    ...
    children: [
      { id: 'requests', label: 'Requests', icon: CheckCircleIcon, path: '/dashboard/super-admin/requests' },
      { id: 'active-leases', label: 'Active Leases', icon: DocumentCheckIcon, path: '/dashboard/super-admin/active-lease' },
      { id: 'renewals', label: 'Lease Renewals', icon: ArrowPathIcon, path: '/dashboard/super-admin/renewals' },
    ]
  },
  { 
    id: 'users', 
    ...
    children: [
      { id: 'users', label: 'Users', icon: UserGroupIcon, path: '/dashboard/super-admin/users' },
      { id: 'add-admin', label: 'Add Admin', icon: PlusCircleIcon, path: '/dashboard/super-admin/add-admin' },
    ]
  },
  { 
    id: 'settings', 
...
export default function SuperAdminSidebar({ onTabChange }: SidebarProps) {
      ...
      <>
        <aside className={`fixed inset-y-0 left-0 z-40 border-r-4 border-emerald-800 bg-gradient-to-b from-emerald-900 to-green-900 text-white transition-all duration-300 hidden md:block ${
          ...
          <div className="flex flex-col items-center py-6">
            {!collapsed ? (
              <Link href="/dashboard/super-admin" className="flex items-center justify-center px-4">
                <Image 
                  src={logo} 
          ...
          {!collapsed && (
            <div className="absolute bottom-6 left-0 right-0 px-4">
              <div className="text-center">
                <p className="text-xs text-white/70">v1.0.0 • Super Admin</p>
    ...
    <>
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-72 bg-gradient-to-b from-emerald-900 to-green-900 text-white transform transition-transform duration-300 ease-in-out md:hidden ${
        ...
        <div className="flex items-center justify-between p-4 border-b border-emerald-800">
          <Link href="/dashboard/super-admin" className="flex items-center" onClick={() => setMobileOpen(false)}>
